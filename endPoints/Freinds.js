...
FriendRouter.post("", (req, res) => {
  ...
  FriendRequests.create(senderRequest, recipientRequest)
    .then(result => {
      ...
      const senderPromise = Users.updateOne(
        { _id: sender.sender },
        { $push: { requests: sender._id } }
      );

      const recipientPromise = Users.updateOne(
        { _id: recipient.sender },
        { $push: { requests: recipient._id } }
      );
...
FriendRouter.put("", (req, res) => {
  ...
  const senderPromise = FriendRequests.updateOne(
  ...
  const recipientPromise = FriendRequests.updateOne(
...
FriendRouter.delete("", (req, res) => {
  ...
  const senderPromise = FriendRequests.findOneAndDelete({
    sender: senderID,
    ...
    if (result.status === "Friends") {
      ...
      return Users.updateOne(
        { _id: senderID },
        { $pull: { requests: result._id } }
      );
    }
  });

  const recipientPromise = FriendRequests.findOneAndDelete({
    sender: recipientID,
    recipient: senderID
  }).then(result => {
    if (result.status === "Friends") {
